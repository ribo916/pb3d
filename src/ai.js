...
import { COURT, accelV2 } from './physics.js';
...
// `timing` is the sigma of the CPU's per-shot contact-timing offset, feeding
// Shots.applyTiming — a lower tier mis-times more, adding organic direction skew
// + pace loss on top of `err` aim scatter.
export const LEVELS = {
...
// far side), or null. Strategies never touch flight internals directly.
//   ball.flight set → exact solver landing, peak from cached apexY.
//   no cached flight (post-bounce roll-out, pop-ups): forward-integrate.
export function predict(ball) {
  if (!ball.live) return null;

  // Fast path: cached solver flight — exact landing + apex, O(1).
  if (ball.flight) {
    var fl = ball.flight;
    return { x: fl.landing.x, z: fl.landing.z, tLeft: Math.max(0, (fl.T || 0) - (fl.elapsed || 0)), peakY: fl.apexY };
  }

  // Fallback: forward integration under the same forces as live flight so
  // post-bounce prediction matches.
 ...
 * The trajectory is passed as `path` = { samples, landing } — pre-sampled flight
 * points + landing.
 * Difficulty-scaled:
 ...
 * { target:{x,z}, apex, spin:{x,y,z} } to feed the shot solver.
